 ...
 */
public class regAsEngServ extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String msgEng = "";
        DB db = new DB();
        boolean f3 = db.boolSelectByMailEng(email);
        boolean f4 = db.boolSelectByMailAll(email);

        if (f3 == false || f4 == false) {
            boolean f1 = db.insertIntoEngineers(fname, lname, speee, loc, fees, address, phone, email, password, imgname,0);
            boolean f2 = db.insertIntoAll(fname, lname, speee, loc, fees, address, phone, email, password, imgname,0);
            if (f1 == true && f2 == true) {

                msgEng = "Regestier Complete Sucssfully.... going to login";
            } else {
                msgEng = "Proplem in register ... try again..!";
            }
        } else {
            msgEng = "Please try with another mail";
        }
        request.setAttribute("msgEng", msgEng);
        RequestDispatcher rd = request.getRequestDispatcher("regAsEng.jsp");
