 ...
 */
public class regAsTechServ extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        MultipartRequest mp = new MultipartRequest(request, getServletContext().getRealPath("/imagees/"),200 *2048 * 2048,new FileRenamePolicy() {
            ...
            String msgTeach = "";
            DB db = new DB();
       boolean f3 = db.boolSelectByMailTeach(email);
        boolean f4 = db.boolSelectByMailAll(email);

        if (f3 == false || f4 == false) {
            boolean f1 = db.insertIntoTeacher(fname, lname, speee, loc, fees, address, phone, email, password, imgname,0);
            boolean f2 = db.insertIntoAll(fname, lname, speee, loc, fees, address, phone, email, password, imgname,0);
            if (f1 == true && f2 == true) {

                msgTeach= "Regestier Complete Sucssfully.... going to login";
            } else {
                msgTeach = "Proplem in register ... try again..!";
            }
        } else {
            msgTeach = "Please try with another mail";
        }
            request.setAttribute("msgTeach", msgTeach);
            RequestDispatcher rd = request.getRequestDispatcher("regAsTech.jsp");
