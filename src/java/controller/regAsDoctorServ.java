 ...
 */
public class regAsDoctorServ extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        boolean f3 = db.boolSelectByMailDoc(email);
        boolean f4 = db.boolSelectByMailAll(email);

        if (f3 == false || f4 == false) {
            boolean f1 = db.insertIntoDoctors(fname, lname, speee, loc, fees, address, phone, email, password, imgname,0);
            boolean f2 = db.insertIntoAll(fname, lname, speee, loc, fees, address, phone, email, password, imgname,0);
            if (f1 == true && f2 == true) {

                msg = "Regestier Complete Sucssfully.... going to login";
            } else {
                msg = "Proplem in register ... try again..!";
            }
        } else {
            msg = "Please try with another mail";
        }
        request.setAttribute("msg", msg);
        RequestDispatcher rd = request.getRequestDispatcher("regAsDoctor.jsp");
