...
input[type="submit"]{
  ...
  margin: -36px auto;
         ...
         <%
             DB db = new DB();
             specialist = (String)request.getAttribute("sppEng");
             location = (String) request.getAttribute("loccEng");
             set = db.selectbyWhereEng(specialist, location);
             boolean f = db.boolEngSearch(specialist, location);
         %>
    </head>
    <body>
        ...
        <!-- start person body-->
          <section class="person-in-city">
           <div class="container">
             ...
             <h2>Best Engineers In your location .. </h2>
             
             <%while (set.next()) {%>
             <div class="person">
               <img src="imagees/<%= set.getString("nimage") %>">
               <span>Eng</span> <span> <%=set.getString("fname")%> &nbsp; <%=set.getString("lname")%> </span>
               <ul>
               ...
               <form action="rateEng?mail=<%=set.getString("mail") %>" method="POST">
