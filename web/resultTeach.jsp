...
input[type="submit"]{
  ...
  margin: -36px auto;
         ...
         <%
             DB db = new DB();
             specialist = (String)request.getAttribute("sppTeach");
             location = (String) request.getAttribute("loccTeach");
             set = db.selectbyWhereTeach(specialist, location);
             boolean f = db.boolTeachSearch(specialist, location);
         %>
    </head>
    <body>
        ...
        <!-- start person body-->
          <section class="person-in-city">
           <div class="container">
             ...
             <h2>Best Teatchers In your location .. </h2>
             
             <%while (set.next()) {%>
             <div class="person">
               <img src="imagees/<%= set.getString("nimage") %>">
               <span>Teatcher </span> &nbsp; <span> <%=set.getString("fname")%> &nbsp; <%=set.getString("lname")%> </span>
               <ul>
               ...
               <form action="rateTeach?mailll=<%=set.getString("mail") %>" method="POST">
