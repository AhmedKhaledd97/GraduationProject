...
input[type="submit"]{
  ...
  margin: -36px auto;
         ...
         <%
             DB db = new DB();
             specialist = (String)request.getAttribute("sppLaw");
             location = (String) request.getAttribute("loccLaw");
             set = db.selectbyWhereLaw(specialist, location);
             boolean f = db.boolLawSearch(specialist, location);
         %>
    </head>
    <body>
        ...
        <!-- start person body-->
          <section class="person-in-city">
           <div class="container">
             ...
             <h2>Best Lawyers In your location .. </h2>
             
             <%while (set.next()) {%>
             <div class="person">
               <img src="imagees/<%= set.getString("nimage") %>">
               <span>Lawyer</span> <span> <%=set.getString("fname")%> &nbsp; <%=set.getString("lname")%> </span>
               <ul>
               ...
               <form action="rateLaw?maill=<%=set.getString("mail") %>" method="POST">
